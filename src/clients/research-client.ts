...
async function resolveUsername(): Promise<string> {
  ...
  const answer = (await rl.question("Enter the X/Twitter username to research: ")).trim();
  rl.close();

  if (!answer) {
    console.error("❌ Username is required to run the research client.");
    process.exit(1);
  }
...
async function main() {
  ...
  console.log("\n" + "=".repeat(80));
  console.log("🎬 STARTING USER RESEARCH PAYMENT FLOW");
  console.log("=".repeat(80));

  try {
    const username = await resolveUsername();

    const response = await api.post("/api/research/user", {
      username: username,
    });
