...
// Clarity security knowledge base
...
interface ContractAnalysisRequest {
  contractIdentifier?: string;  // e.g., "SP000.contract-name"
  contractCode?: string;        // Raw Clarity code
  contractName?: string;
}
...
// Fetch contract source code from Stacks API
...
Analyze ALL security aspects following the multi-stage framework.`,
  ...
  try {
    ...
    console.error(`  Raw content:`, typeof content === "string" ? content.substring(0, 500) : JSON.stringify(content).substring(0, 500));
    analysis = {
      overallRisk: "high",
...
// POST /audit - Comprehensive security analysis
app.post(
  "/audit",
  x402PaymentRequired({
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Fetch from Stacks API if identifier provided, otherwise use provided code
      ...
      console.log(
        `  Audit complete: ${auditResult.overallRisk} risk, ${auditResult.vulnerabilities.length} findings`
      );
      ...
      // Better error handling for OpenRouter issues
...
app.get("/health", (_req: Request, res: Response) => {
  res.json({
    status: "ok",
    service: "WHITEHAT - Clarity Security Scanner",
...
app.listen(PORT, () => {
  console.log(`\n🛡️  WHITEHAT - Clarity Security Scanner`);
  ...
  console.log("  POST /audit - Comprehensive security analysis (0.02 STX)");
  console.log("");
});
